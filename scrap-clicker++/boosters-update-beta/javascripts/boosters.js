...
function getBoosterGain() {
    return Decimal.floor(
        player.scrap.min(getBoosterSlowdown()).div("1e55000").pow(Decimal.div(1, 50000))
        .times(Decimal.log(player.scrap.max(getBoosterSlowdown()).minus(getBoosterSlowdown()).div(getBoosterSlowdown()).plus(10), 10).pow(1.25))
        .times(
...
function BCActiveText2(i) {
    if (player.boosterChallenges.isActive[i]) {
        return "Exit<br>this challenge"
    } else {
        return "Enter<br>this challenge"
    }
}
