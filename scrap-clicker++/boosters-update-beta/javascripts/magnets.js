...
function getMagnetBoosts() {
    return (player.electrons.div(100).plus(1)
        ...
        .pow( (player.magnetUpgrades.nonRepeatable.isBought[15]) ? Decimal.log(player.protons.pow(0.01).plus(10), 10).min(10) : 1 )
    )
    .times(
...
function getSteelBeamBoosts() {
    return (player.protons.div(100).plus(1)
        .pow(player.brickUpgrades.repeatable.level[2].times(0.01).plus(1))
        .pow( (player.magnetUpgrades.nonRepeatable.isBought[15]) ? Decimal.log(player.electrons.pow(0.01).plus(10), 10).min(10) : 1 )
    )
    .times(
        (getTotalLevelsOfRepeatableMagnetUpgrade(0).div(5).plus(1).pow(3)).times(getTotalLevelsOfRepeatableMagnetUpgrade(0).plus(1))
        .pow( (player.fuelUpgrades.isBought[0]) ? 1.5 : 1 )
...
function produceEachOther(diff) {
    player.magnets = player.magnets.add(
        (Decimal.log(player.steelBeams.plus(1).plus(player.savingMilestones.isGotten[5] ? 0.05 : 0), 3).max(0)).pow(1.35).div(2).times(diff).times(getMagnetBoosts())
    )
    player.steelBeams = player.steelBeams.add(
        (Decimal.log(player.magnets.plus(1).plus(player.savingMilestones.isGotten[5] ? 0.05 : 0), 3).max(0)).pow(1.35).div(2).times(diff).times(getSteelBeamBoosts())
    )
    player.magnets = player.magnets.add(
        (Decimal.log(player.steelBeams.plus(1).plus(player.savingMilestones.isGotten[5] ? 0.05 : 0), 3).max(0)).pow(1.35).div(2).times(diff).times(getMagnetBoosts())
    )
