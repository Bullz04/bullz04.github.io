...
function increaseAutoDids(diff) {
    if (player.automator[0].isActive) {
        player.automator[0].dids = player.automator[0].dids.add(
            ...
            .times(Decimal.log(player.goldenScrap.plus(2), 2).pow(2))
            .times(diff)
        )
    }
...
function upgradeAutomator(amount, i) {
    if (amount == "singles") {
        ...
        if (player.automator[i].level.lt(player.automator[i].maxLevel)) {
            while (player.goldenScrap.gte(player.automator[i].cost.div(getBoosterPointTypeEffects(1)).times(Decimal.pow(player.automator[i].costScaling, player.automator[i].level)))) {
                let amounty = Decimal.min(Decimal.affordGeometricSeries(player.goldenScrap, player.automator[i].cost.div(getBoosterPointTypeEffects(1)), player.automator[i].costScaling, player.automator[i].level), player.automator[i].maxLevel)
