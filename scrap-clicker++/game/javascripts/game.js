...
function getScrapGain() {
    ...
    .times(
        (
            Decimal.pow(
                player.timeSpentInThisPrestige.plus((player.goldenScrapUpgrades.isBought[11]) ? 1e12 : 0).plus(1),
...
function updateText() {
    //document.getElementById("PossibleBoughtGSULeftText")
    ...
    */
    ...
    for (i = 0; i < 8; i++) {
        document.getElementById("BCButtonText" + (i + 1)).innerHTML = BCActiveText2(i)
    }
    ...
    document.getElementById("TrueBCCompletions8").innerHTML = ( (getTrueBCCompletions(7).gte(1)) ? format(player.brickUpgrades.repeatable.level[1].plus(1).pow(getTrueBCCompletions(7).max(1).times(3)), 3, 1) : "???" )
...
//autoNMU
    ...
    for (i = 0; i < 2; i++) {
        ...
        document.getElementById("AutomatorSpeed" + (i + 1)).innerHTML = format(player.automator[i].baseSpeed.times( Decimal.pow(player.automator[i].speedPerLevel, player.automator[i].level) ).times(Decimal.log(player.goldenScrap.plus(2), 2).pow(2))      , 3, 2)
