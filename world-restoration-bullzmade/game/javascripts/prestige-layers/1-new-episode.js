function getKeyGain() {
    let boost = function() {
        ...
        .times( (player.cakeUpgrades.nonRepeatable.isBought[5]) ? getNCUpgradeEffect("6-Keys") : new Decimal(1) )
    ...
    return Decimal.floor(
        player.repelledForks.div(1000).root(2)
        .times(boost())
        .times( (eval(temp.prestigeRequirements.resource[0]).gte(temp.prestigeRequirements.amountRequired[0])) ? 1 : 0 )
    )
}
...
function getRNEUpgradeEffect(text) {
    switch (text+"") {
        case "1":
            return Decimal.log(player.keys.plus(2), 2).root(5)
        ...
        case "2":
            return Decimal.log(player.repelledForks.plus(2), 2).pow(player.NEUpgrades.repeatable.levels[1].times(0.25))
        case "3":
            return Decimal.log(player.repelledForks.plus(10), 10).root(4)
...
function getNNEUpgradeEffect(text) {
    switch (text+"") {
        case "1":
            return Decimal.log(player.gainedKeys.plus(10), 10).pow(4)
            .times(player.gainedKeys.div(1e5).plus(1).root(2.5))
        case "2":
        ...
        case "7":
            return Decimal.log(Decimal.log(player.keys.plus(10), 10).plus(9), 10)
        case "8":
...
function getRNEUScalingWeakness(i) {
    switch (i+1) {
        case 1:
            ...
            .times( (player.achievementsGotten[22]) ? getAchRewards("23") : new Decimal(1) )
        case 2:
            return new Decimal(1)
        ...
        case 3:
            return new Decimal(1)
            .times( (player.NEUpgrades.nonRepeatable.isBought[4]) ? getNNEUpgradeEffect("5") : new Decimal(1) )
            .times( (player.cakeUpgrades.nonRepeatable.isBought[3]) ? getNCUpgradeEffect("4") : new Decimal(1) )
            .times( (player.karmaUpgrades.nonRepeatable.isBought[9]) ? getNKaUpgradeEffect("10") : new Decimal(1) )
            .times( (player.achievementsGotten[22]) ? getAchRewards("23") : new Decimal(1) )
